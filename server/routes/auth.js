...
router.get("/email-alert-count", auth, async (req, res) => {
  try {
    ...
    const subs = await Subscriber.find({ email })
      .select("email_alerts_sent")
      .lean();
    const total = subs.reduce(
      (sum, s) => sum + (Number(s.email_alerts_sent) || 0),
      0,
    );
    res.json({ total });
  } catch (err) {
  ...
  try {
    ...
    if (authHeader && authHeader.startsWith("Bearer ")) {
      try {
        ...
        const refUser = await User.findById(decoded.user.id).select(
          "name email",
        );
        if (refUser && refUser.email) {
          ...
          if (refLower !== inviteeLower) {
            const displayName =
              refUser.name && refUser.name.trim()
                ? refUser.name.trim()
    ...
    const magicToken = jwt.sign(payload, process.env.JWT_SECRET, {
      expiresIn: "1h",
    });
...
router.get("/magic-login", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(400)
      .send("Link has expired or is invalid. Please try subscribing again.");
  }
});
