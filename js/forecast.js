/*
   ...
   Fetches XGBoost forecasts from the Flask API 
*/
...
function updateSummaryCards(forecast) {
  ...
  // 24-hour stats
  const avg24Val = Math.round(
    first24.reduce((s, f) => s + f.aqi, 0) / first24.length,
  );
  ...
  // 48-hour stats
  const avg48Val = Math.round(
    full48.reduce((s, f) => s + f.aqi, 0) / full48.length,
  );
...
function updateAlertBanner(forecast) {
  ...
  if (maxAqi > 150) {
    ...
    if (maxAqi > 300) {
      ...
      if (msgEl)
        msgEl.textContent =
          "Everyone should avoid all outdoor exertion. Stay indoors with air purification.";
    } else if (maxAqi > 200) {
      if (titleEl)
        titleEl.textContent = "⚠ Very Unhealthy Air Quality Expected";
      if (msgEl)
        msgEl.textContent =
          "Everyone may experience health effects. Sensitive groups should stay indoors.";
    } else {
      if (titleEl)
        titleEl.textContent = "⚠ High Pollution Expected in the Next 24 Hours";
      if (msgEl)
        msgEl.textContent =
      ...
      datasets: [
        {
          ...
          pointHoverBackgroundColor: (ctx) =>
            segColors[ctx.dataIndex] || "#7c3aed",
...
function initHealthTimeline(forecast, live) {
  ...
  if (todayItems.length === 0) {
    container.innerHTML =
      '<p style="text-align:center; color:var(--text-secondary); font-family:Inter,sans-serif; padding:1rem;">No more forecast data for today.</p>';
  ...
  for (let start = 0; start < aqiArr.length; start++) {
    for (
      let end = start + 1;
      end <= Math.min(start + 6, aqiArr.length);
      end++
    ) {
  ...
  grid.querySelectorAll(".station-card").forEach((card) => {
    card.addEventListener("click", () => {
      ...
      grid
        ...
        .forEach((c) => c.classList.remove("station-card-active"));
...
function showError(msg) {
  ...
  if (msgEl)
    msgEl.textContent = msg || "Could not connect to the forecast API.";
