...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (learnMoreBtn) {
    learnMoreBtn.addEventListener("click", function () {
      ...
      const moreInfoLink = Array.from(
        document.querySelectorAll(".nav-link"),
      ).find((link) => link.textContent === "More Info");
      if (moreInfoLink) {
...
async function loadAirQualityData() {
  ...
  try {
    ...
    hideLoadingState();
    showErrorState(
      `Unable to load air quality data: ${error.message}. Please check your internet connection and API configuration.`,
    );
...
function startRealTimeUpdates() {
  ...
  const interval =
    typeof API_CONFIG !== "undefined" && API_CONFIG.updateInterval
